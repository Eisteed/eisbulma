(function initAjaxSearch() {
  ...
  dropdown.insertAdjacentHTML(
    ...
    '<div class="dropdown-content mr-auto ml-auto" style="word-wrap: break-word; overflow-wrap: break-word; max-width:1280px;"></div>' +
    '</div>'
  );
    ...
    try {
      ...
      if (response && response.success) {
        ...
        if (!append) {
          ...
          const term = response.data.search_term;

          const msg = eisbulma_i18n.found_results
            .replace('{count}', count)
            .replace('{term}', term);

          html += '<div class="dropdown-item">';
          html += `<div class="message is-success is-small mb-3">
                   <div class="message-body py-2">${msg}</div>
                    </div>`;
          html += '</div>';
        }
